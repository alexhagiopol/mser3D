...
void drawAndWriteEllipse(const MSER::Region & region, int width, int height, int depth,
				 vector<uint8_t> & bits, const uint8_t * color, ofstream& myfile)
{
	// Centroid (mean)
...
int main(int argc, const char * argv[])
{
	...
	mser4_1(&grayscale[0], width, height, regions[0]);
	
	// Invert the pixel values
	for (int i = 0; i < width * height; ++i)
		grayscale[i] = ~grayscale[i];
	
	mser4_2(&grayscale[0], width, height, regions[1]);
	...
	for (int i = 0; i < 2; ++i)
		for (int j = 0; j < regions[i].size(); ++j)
			drawAndWriteEllipse(regions[i][j], width, height, depth, original, colors[i], myfile);
